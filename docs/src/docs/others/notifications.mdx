...
### A Notification driven by URL params

We're using Remix `useSearchParams` hook to get URL search params in this example.
```tsx
import { useEffect } from "react";
import { useSearchParams } from "@remix-run/react";

import { hideNotification, showNotification, updateNotification, useNotifications } from "@mantine/notifications";
...
We set `autoClose` to `false` to make sure that the URL is the only variable in control.
Now if you navigate to `?message=message&type=warning`, you will see the notification until you navigate to something else.
